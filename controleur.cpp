...
void Controleur::newNiveau(){
    QList<Case*> liste;
    QQmlComponent component1(viewer->engine(),QUrl::fromLocalFile("qml/SweetCandy/VueCase.qml"));
    QQmlComponent component2(viewer->engine(),QUrl::fromLocalFile("qml/SweetCandy/VueBonbon.qml"));
    QQuickItem* grille = viewer->rootObject()->findChild<QQuickItem *>("grilleDeJeux");
    if(grille!=NULL){
        for(int i=0;i<9;i++){
            for(int j=0;j<9;j++){
